...
def survey(ax, country_data, answer_categories):
	...
	ax.legend(ncol=len(answer_categories), bbox_to_anchor=(-0.005, -0.05),
			  loc='center left', fontsize='12')


def data_by_gender(ax_nr, answer_categories, data_men, data_women, title):
	"""creates grouped horizontal bar chart with some formatting options"""
...
def data_by_education(ax_nr, answer_categories, ed_02, ed_34, ed_58, title):
	...
	rects1 = ax_nr.barh(y_pos - width, ed_02, width, color='#9C9EDE',
						label = 'Lower secondary or less')
	rects2 = ax_nr.barh(y_pos, ed_34, width, color='#5254A3',
						label = 'Upper and post secondary')
	rects3 = ax_nr.barh(y_pos + width, ed_58, width, color='#393B79',
						label = 'Tertiary')
...
def data_by_age(ax_nr, answer_categories, y15_24, y25_34, y35_44, y45_54,
				y55_64, y65_more, title):
	"""creates grouped horizontal bar chart with some formatting options"""
	y_pos = np.arange(len(answer_categories))
	width = 0.12

	rects1 = ax_nr.barh(y_pos - width*2.5, y15_24, width, color = '#CEDB9C',
						label = 'y15_24')
	rects2 = ax_nr.barh(y_pos - width*1.5, y25_34, width, color = '#B4CF6A',
						label = 'y25_34')
	rects3 = ax_nr.barh(y_pos - width/2, y35_44, width, color = '#E7BA52',
						label = 'y35_44')
	rects4 = ax_nr.barh(y_pos + width/2, y45_54, width, color = '#BD9E39',
						label = 'y45_54')
	rects5 = ax_nr.barh(y_pos + width*1.5, y55_64, width, color = '#E7969C',
						label = 'y55_64')
	rects6 = ax_nr.barh(y_pos + width*2.5, y65_more, width, color ='#D6616B',
						label = 'y65_more')
